...
- Generate detailed Excel reports of the analysis (optional).
---
...
The following table highlights the checks performed by `pfShell`:

| Severity  | Check Description                                                                                         |
|-----------|----------------------------------------------------------------------------------------------------------|
| High      | Rule allows packets to any destination on any service/port.                                               |
| High      | Rule allows any source, any destination, and multiple services/ports.                                     |
...
| Medium    | Rule allows packets through a large service/port range (over 1000 ports).                                 |
| Medium    | SNMP enabled with a default/weak community string.                                                        |
| Low       | Reject rule identified.                                                                                   |
| Low       | SNMP configuration uses a secure community string but is less secure than SNMPv3.                        |
